...
$$
b_0 = C \sum_{i=1}^n \Delta t \, e^{-(r+\lambda) T_i}
= -(1 - \delta) \sum_{i=1}^n \mathbb{E}\left[ e^{-r T_i} \left( e^{-\lambda T_i} - e^{-\lambda T_{i-1}} \right) \right]
= a_0
$$
...
Just like that we have "fair" prices for each tranche in Bob's CDO. You now have the tools to understand and model what Micheal Burry did when he bought a credit default swap on a collateralized debt obligation! 
